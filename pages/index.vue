<template>
  <div class="flex mb-4 h-screen">
    <div class="w-full md:w-1/4 bg-gray-400 p-4 text-center text-gray-700 h-screen"> 
      <input class="appearance-none bg-transparent border-none w-full text-white mr-3 py-1 px-2 leading-tight focus:outline-none" v-model="limite" id="limit" placeholder="Limite de resultados">
    <!--
      Esta es la barra de la izquierda, donde se va a agregar la busqueda de plataformas. ETC
    -->
  
    </div>
    <div class="w-full md:w-3/4 bg-gray-500 p-4 text-center text-gray-200 h-screen">
      <!-- 
      ...
      <div class="border-b border-2 border-gray-500 py-2 bg-gray-400 px-2">
        <div>
        ...
        <div class="game-container flex flex-wrap border-3 border-gray-500 h-screen">
          <nuxt-link :to="'/games/' + game.id" v-for="game in games" :key="game.id" class="w-full md:w-1/5">
