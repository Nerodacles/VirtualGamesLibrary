  ...
  <div class="flex h-screen">
    <div class="w-full md:w-1/4 p-4 text-center text-gray-700 h-full">
      ...
      <div class="flex items-center py-2 px-2">
        ...
        <button v-on:click="popularGames, platform_id = null" class="text-white">Popular Games</button>
      </div>
      <div class="flex items-center py-2 px-2">
        <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px"
          width="32" height="32"
    ...
    <div class="w-full md:w-3/4 p-4 text-center text-gray-200 h-full">
      ...
      <div class="py-2 px-2 h-full">
        <div>
          <h1 class="font-heading uppercase mb-1 text-white" v-if="(!keyword) && (!platform_id)">Popular Games</h1>  
          <h1 class="font-heading uppercase mb-1 text-white" v-if="(keyword) && (!platform_id)">Search game {{ keyword }}</h1>
  ...
  methods: {
    searchGames() {
      axios.get("https://rawg-video-games-database.p.rapidapi.com/games?ordering=-relevance&discover=true",  // Se empleza con la llamada a la api para buscar los juegos.
        { params: { search: this.keyword, platforms: this.platform_id } })
        .then(res => { this.games = res.data.results } )
    ...
    searchPlatform(id) {
      axios.get("https://rawg-video-games-database.p.rapidapi.com/games/lists/main?ordering=-relevance", // Se empleza con la llamada a la api para obtener todos los id de las plataformas
        { params: { platforms: id, page_size: this.limite } })
        .then(res => { this.games = res.data.results } )
        .catch(err => { console.log(err) } );
      this.platform_id = id;
      this.keyword =  null;
